...
unsigned int temperature, pressure, altitude, alt;
...
unsigned int bmp085_Altitude(float pressure, float temperature)
{
  ...
  /* double _ph0 = 1013.25;
   double _h0 = 0;
   double pres = pressure / 100.0;

   double p1 = pres / _ph0;
   double p2 = pow(p1, 1.0 / 5.255);
   double p3 = (p2 - 1);
   ...
   float temperatur = temperature / 10;
   float p = pres / ( 287.058 * temperatur);
   float tmp = 1013.25 - p * 9.81;
   float altitude = pres / tmp;
